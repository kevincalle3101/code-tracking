...
export class RedisAdapter {
    ...
    constructor(private readonly configService: ConfigService) {
        this.ttl = this.configService.get<number>('redis.ttl', 604800); // 7 days default
    ...
    async get<T>(key: string): Promise<T | null> {
        try {
            ...
            if (value) {
                this.logger.log(`Cache encontrada para clave: ${key}`);
                return JSON.parse(value) as T;
            }
            this.logger.log(`Cache no encontrada para clave: ${key}`);
            return null;
        } catch (error) {
